...
public class Update{
    ...
    protected Update(String URL) {
        try {
            ...
            isr.close();
            in.close();
            if (Objects.equals(Main.plugin.getDescription().getVersion(), str)) {
                //为最新版本
                ...
                new Message("最新版本:" + str + ",当前版本:" + Main.plugin.getDescription().getVersion()).warn();
            ...
            new Message("下载最新版本链接: https://www.eggpixel.cn/").error();
            if (!Main.plugin.getServer().getBukkitVersion().startsWith("1.16")) {
                new Message("当前版本并不在支持版本内，发生任何错误不会获得任何支持").error();
            }
        }
